...
func toList(clist C.CK_ULONG_PTR, size C.CK_ULONG) []uint64 {
	l := make([]uint64, int(size))
	for i := 0; i < len(l); i++ {
		l[i] = uint64(C.Index(clist, C.CK_ULONG(i)))
	}
...
func toVersion(version C.CK_VERSION) *pkcs11.Version {
...
// byte slices.
func NewAttribute(typ uint64, x interface{}) *pkcs11.Attribute {
...
func cAttributeList(a []*pkcs11.Attribute) (arena, C.CK_ATTRIBUTE_PTR, C.CK_ULONG) {
	var arena arena
	if len(a) == 0 {
		return nil, nil, 0
	}
...
func NewMechanism(mech uint64, x interface{}) *pkcs11.Mechanism {
	m := new(pkcs11.Mechanism)
	m.Mechanism = mech
	if x == nil {
		return m
	}
...
func cMechanism(mechList []*pkcs11.Mechanism) (arena, *C.CK_MECHANISM) {
	if len(mechList) != 1 {
