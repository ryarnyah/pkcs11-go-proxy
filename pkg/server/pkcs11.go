...
func (c *Ctx) Finalize() error {
	if c.ctx == nil {
		return toError(pkcs11.CKR_CRYPTOKI_NOT_INITIALIZED)
	}
	e := C.Finalize(c.ctx)
	return toError(e)
}
...
func (c *Ctx) GetInfo() (*pkcs11.Info, error) {
	var p C.ckInfo
	e := C.GetInfo(c.ctx, &p)
	i := &pkcs11.Info{
...
func (c *Ctx) GetSlotList(tokenPresent bool) ([]uint64, error) {
	var (
...
func (c *Ctx) GetSlotInfo(slotID uint) (*pkcs11.SlotInfo, error) {
...
func (c *Ctx) GetTokenInfo(slotID uint) (*pkcs11.TokenInfo, error) {
	...
	s := &pkcs11.TokenInfo{
		...
		Flags:              uint64(cti.flags),
		MaxSessionCount:    uint64(cti.ulMaxSessionCount),
		SessionCount:       uint64(cti.ulSessionCount),
		MaxRwSessionCount:  uint64(cti.ulMaxRwSessionCount),
		RwSessionCount:     uint64(cti.ulRwSessionCount),
		MaxPinLen:          uint64(cti.ulMaxPinLen),
		MinPinLen:          uint64(cti.ulMinPinLen),
		TotalPublicMemory:  uint64(cti.ulTotalPublicMemory),
		FreePublicMemory:   uint64(cti.ulFreePublicMemory),
		TotalPrivateMemory: uint64(cti.ulTotalPrivateMemory),
		FreePrivateMemory:  uint64(cti.ulFreePrivateMemory),
...
func (c *Ctx) GetMechanismList(slotID uint) ([]*pkcs11.Mechanism, error) {
	var (
	...
	m := make([]*pkcs11.Mechanism, int(mechlen))
...
func (c *Ctx) GetMechanismInfo(slotID uint, m []*pkcs11.Mechanism) (*pkcs11.MechanismInfo, error) {
	...
	mi := &pkcs11.MechanismInfo{
		MinKeySize: uint64(cm.ulMinKeySize),
		MaxKeySize: uint64(cm.ulMaxKeySize),
		Flags:      uint64(cm.flags),
	}
	return mi, toError(e)
}
...
func (c *Ctx) InitPIN(sh pkcs11.SessionHandle, pin string) error {
	p := C.CString(pin)
	defer C.free(unsafe.Pointer(p))
...
func (c *Ctx) SetPIN(sh pkcs11.SessionHandle, oldpin string, newpin string) error {
...
func (c *Ctx) OpenSession(slotID uint, flags uint) (pkcs11.SessionHandle, error) {
	...
	return pkcs11.SessionHandle(s), toError(e)
}
...
func (c *Ctx) GetSessionInfo(sh pkcs11.SessionHandle) (*pkcs11.SessionInfo, error) {
	var csi C.CK_SESSION_INFO
	e := C.GetSessionInfo(c.ctx, C.CK_SESSION_HANDLE(sh), &csi)
	s := &pkcs11.SessionInfo{SlotID: uint64(csi.slotID),
		State:       uint64(csi.state),
		Flags:       uint64(csi.flags),
		DeviceError: uint64(csi.ulDeviceError),
	}
	return s, toError(e)
}

// GetOperationState obtains the state of the cryptographic operation in a session.
func (c *Ctx) GetOperationState(sh pkcs11.SessionHandle) ([]byte, error) {
	var (
		state    C.CK_BYTE_PTR
		statelen C.CK_ULONG
	)
...
func (c *Ctx) SetOperationState(sh pkcs11.SessionHandle, state []byte, encryptKey, authKey pkcs11.ObjectHandle) error {
...
func (c *Ctx) CreateObject(sh pkcs11.SessionHandle, temp []*pkcs11.Attribute) (pkcs11.ObjectHandle, error) {
...
func (c *Ctx) CopyObject(sh pkcs11.SessionHandle, o pkcs11.ObjectHandle, temp []*pkcs11.Attribute) (pkcs11.ObjectHandle, error) {
	var obj C.CK_OBJECT_HANDLE
	arena, t, tcount := cAttributeList(temp)
	defer arena.Free()
...
func (c *Ctx) GetAttributeValue(sh pkcs11.SessionHandle, o pkcs11.ObjectHandle, a []*pkcs11.Attribute) ([]*pkcs11.Attribute, error) {
	...
	a1 := make([]*pkcs11.Attribute, len(a))
	for i, c := range pa {
...
func (c *Ctx) SetAttributeValue(sh pkcs11.SessionHandle, o pkcs11.ObjectHandle, a []*pkcs11.Attribute) error {
...
func (c *Ctx) FindObjectsInit(sh pkcs11.SessionHandle, temp []*pkcs11.Attribute) error {
	arena, t, tcount := cAttributeList(temp)
	defer arena.Free()
...
func (c *Ctx) FindObjects(sh pkcs11.SessionHandle, max int) ([]pkcs11.ObjectHandle, bool, error) {
	...
	o := make([]pkcs11.ObjectHandle, len(l))
	for i, v := range l {
		o[i] = pkcs11.ObjectHandle(v)
	}
...
func (c *Ctx) EncryptInit(sh pkcs11.SessionHandle, m []*pkcs11.Mechanism, o pkcs11.ObjectHandle) error {
...
func (c *Ctx) Encrypt(sh pkcs11.SessionHandle, message []byte) ([]byte, error) {
	var (
		enc    C.CK_BYTE_PTR
		enclen C.CK_ULONG
	)
...
func (c *Ctx) EncryptUpdate(sh pkcs11.SessionHandle, plain []byte) ([]byte, error) {
	var (
		part    C.CK_BYTE_PTR
		partlen C.CK_ULONG
	)
...
func (c *Ctx) EncryptFinal(sh pkcs11.SessionHandle) ([]byte, error) {
	var (
		enc    C.CK_BYTE_PTR
		enclen C.CK_ULONG
	)
...
func (c *Ctx) DecryptInit(sh pkcs11.SessionHandle, m []*pkcs11.Mechanism, o pkcs11.ObjectHandle) error {
	arena, mech := cMechanism(m)
	defer arena.Free()
...
func (c *Ctx) Decrypt(sh pkcs11.SessionHandle, cipher []byte) ([]byte, error) {
	var (
		plain    C.CK_BYTE_PTR
		plainlen C.CK_ULONG
	)
...
func (c *Ctx) DecryptUpdate(sh pkcs11.SessionHandle, cipher []byte) ([]byte, error) {
	var (
		part    C.CK_BYTE_PTR
		partlen C.CK_ULONG
	)
...
func (c *Ctx) DecryptFinal(sh pkcs11.SessionHandle) ([]byte, error) {
	var (
		plain    C.CK_BYTE_PTR
		plainlen C.CK_ULONG
	)
...
func (c *Ctx) DigestInit(sh pkcs11.SessionHandle, m []*pkcs11.Mechanism) error {
	arena, mech := cMechanism(m)
	defer arena.Free()
...
func (c *Ctx) Digest(sh pkcs11.SessionHandle, message []byte) ([]byte, error) {
	var (
		hash    C.CK_BYTE_PTR
		hashlen C.CK_ULONG
	)
...
func (c *Ctx) DigestUpdate(sh pkcs11.SessionHandle, message []byte) error {
...
func (c *Ctx) DigestKey(sh pkcs11.SessionHandle, key pkcs11.ObjectHandle) error {
...
func (c *Ctx) DigestFinal(sh pkcs11.SessionHandle) ([]byte, error) {
	var (
		hash    C.CK_BYTE_PTR
		hashlen C.CK_ULONG
	)
...
func (c *Ctx) SignUpdate(sh pkcs11.SessionHandle, message []byte) error {
...
func (c *Ctx) SignFinal(sh pkcs11.SessionHandle) ([]byte, error) {
	var (
		sig    C.CK_BYTE_PTR
		siglen C.CK_ULONG
	)
...
func (c *Ctx) SignRecoverInit(sh pkcs11.SessionHandle, m []*pkcs11.Mechanism, key pkcs11.ObjectHandle) error {
	arena, mech := cMechanism(m)
	defer arena.Free()
...
func (c *Ctx) SignRecover(sh pkcs11.SessionHandle, data []byte) ([]byte, error) {
	var (
		sig    C.CK_BYTE_PTR
		siglen C.CK_ULONG
	)
...
func (c *Ctx) VerifyInit(sh pkcs11.SessionHandle, m []*pkcs11.Mechanism, key pkcs11.ObjectHandle) error {
	arena, mech := cMechanism(m)
	defer arena.Free()
...
func (c *Ctx) Verify(sh pkcs11.SessionHandle, data []byte, signature []byte) error {
...
// and plaintext cannot be recovered from the signature.
func (c *Ctx) VerifyUpdate(sh pkcs11.SessionHandle, part []byte) error {
...
func (c *Ctx) VerifyFinal(sh pkcs11.SessionHandle, signature []byte) error {
...
func (c *Ctx) VerifyRecoverInit(sh pkcs11.SessionHandle, m []*pkcs11.Mechanism, key pkcs11.ObjectHandle) error {
	arena, mech := cMechanism(m)
	defer arena.Free()
...
// operation, where the data is recovered from the signature.
func (c *Ctx) VerifyRecover(sh pkcs11.SessionHandle, signature []byte) ([]byte, error) {
	var (
		data    C.CK_BYTE_PTR
		datalen C.CK_ULONG
	)
...
func (c *Ctx) DigestEncryptUpdate(sh pkcs11.SessionHandle, part []byte) ([]byte, error) {
	var (
		enc    C.CK_BYTE_PTR
		enclen C.CK_ULONG
	)
...
func (c *Ctx) DecryptDigestUpdate(sh pkcs11.SessionHandle, cipher []byte) ([]byte, error) {
	var (
		part    C.CK_BYTE_PTR
		partlen C.CK_ULONG
	)
...
func (c *Ctx) SignEncryptUpdate(sh pkcs11.SessionHandle, part []byte) ([]byte, error) {
	var (
		enc    C.CK_BYTE_PTR
		enclen C.CK_ULONG
	)
...
func (c *Ctx) DecryptVerifyUpdate(sh pkcs11.SessionHandle, cipher []byte) ([]byte, error) {
	var (
		part    C.CK_BYTE_PTR
		partlen C.CK_ULONG
	)
...
func (c *Ctx) GenerateKey(sh pkcs11.SessionHandle, m []*pkcs11.Mechanism, temp []*pkcs11.Attribute) (pkcs11.ObjectHandle, error) {
	...
	e1 := toError(e)
	if e1 == nil {
		return pkcs11.ObjectHandle(key), nil
	}
	return 0, e1
}

// GenerateKeyPair generates a public-key/private-key pair creating new key objects.
func (c *Ctx) GenerateKeyPair(sh pkcs11.SessionHandle, m []*pkcs11.Mechanism, public, private []*pkcs11.Attribute) (pkcs11.ObjectHandle, pkcs11.ObjectHandle, error) {
	var (
	...
	e1 := toError(e)
	if e1 == nil {
		return pkcs11.ObjectHandle(pubkey), pkcs11.ObjectHandle(privkey), nil
	}
	return 0, 0, e1
}
...
func (c *Ctx) UnwrapKey(sh pkcs11.SessionHandle, m []*pkcs11.Mechanism, unwrappingkey pkcs11.ObjectHandle, wrappedkey []byte, a []*pkcs11.Attribute) (pkcs11.ObjectHandle, error) {
	var key C.CK_OBJECT_HANDLE
	...
	return pkcs11.ObjectHandle(key), toError(e)
}

// DeriveKey derives a key from a base key, creating a new key object.
func (c *Ctx) DeriveKey(sh pkcs11.SessionHandle, m []*pkcs11.Mechanism, basekey pkcs11.ObjectHandle, a []*pkcs11.Attribute) (pkcs11.ObjectHandle, error) {
	var key C.CK_OBJECT_HANDLE
	attrarena, ac, aclen := cAttributeList(a)
	defer attrarena.Free()
	mecharena, mech := cMechanism(m)
...
func (c *Ctx) SeedRandom(sh pkcs11.SessionHandle, seed []byte) error {
...
func (c *Ctx) GenerateRandom(sh pkcs11.SessionHandle, length int) ([]byte, error) {
...
func (c *Ctx) WaitForSlotEvent(flags uint) (uint64, error) {
	var slotID C.CK_ULONG
	e := C.WaitForSlotEvent(c.ctx, C.CK_FLAGS(flags), &slotID)
	if toError(e) != nil {
		return 0, toError(e)
	}
